...
public final class RecyclerPaginate extends Paginate {
    ...
    NestedScrollView.OnScrollChangeListener customOnScrollChangeListener;

    RecyclerPaginate(NestedScrollView nestedScrollView, RecyclerView recyclerView,
        ...
        this(recyclerView, callbacks, loadingTriggerThreshold, addLoadingListItem, loadingListItemCreator, loadingListItemSpanLookup);
        this.nestedScrollView = nestedScrollView;
        this.nestedScrollView.setOnScrollChangeListener(mOnScrollChangeListener);
    }

    public void setCustomOnScrollChangeListener(NestedScrollView.OnScrollChangeListener customOnScrollChangeListener) {
    ...
    private final NestedScrollView.OnScrollChangeListener mOnScrollChangeListener = new NestedScrollView.OnScrollChangeListener() {
        @Override
        public void onScrollChange(NestedScrollView v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
            if(customOnScrollChangeListener != null){
                customOnScrollChangeListener.onScrollChange(v, scrollX, scrollY, oldScrollX, oldScrollY);
            }
            if (!hasCalledCallback) {
                hasCalledCallback = true;
            }else{
                hasCalledCallback = callbacks.isLoading();
                return;
            }
//            Log.d("RecyclerPaginate","scrollY-> " + scrollY + "\nv.getHeight()->" + (v.getChildAt(0).getMeasuredHeight() - v.getMeasuredHeight()) + "\nv.computeVerticalScrollOffset()->" + v.computeVerticalScrollRange());
            int viewHeight = v.getChildAt(0).getMeasuredHeight() - v.getMeasuredHeight();
                if((viewHeight - scrollY) <= loadingTriggerThreshold){
                    if (!callbacks.isLoading() && !callbacks.hasLoadedAllItems()) {
                        callbacks.onLoadMore();
                    }
                }
    ...
    public static class Builder {
        ...
        public Builder(NestedScrollView nestedScrollView, RecyclerView recyclerView, Paginate.Callbacks callbacks) {
            this(recyclerView, callbacks);
            this.nestedScrollView = nestedScrollView;
        }
         ...
         * if 5.
         *
         * @param threshold number of items from the end of the list / pixel offset to bottom if recyclerview is in nested scroll view.
        ...
        public Paginate build() {
            ...
            if (nestedScrollView != null) {
                return new RecyclerPaginate(nestedScrollView, recyclerView, callbacks, loadingTriggerThreshold, addLoadingListItem,
